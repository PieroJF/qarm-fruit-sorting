# Final Presentation PowerPoint Generator — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate an 8-slide professional PowerPoint (`slides/FinalPresentation_FruitSorting.pptx`) covering the full FruitSorting project end-to-end, matching the Assignment 3 dark-navy aesthetic.

**Architecture:** Single Python script (`slides/generate_final_pptx.py`) using `python-pptx`. Reuses the A3 color palette and helper-function signatures. Auto-inserts images from `figures/` when available, falls back to labeled placeholder boxes. Each slide targets specific marking criteria.

**Tech Stack:** Python 3.13, `python-pptx`
...
| File | Action | Purpose |
|------|--------|---------|
| `slides/generate_final_pptx.py` | Create | Main script — all 8 slides + helpers |
| `slides/FinalPresentation_FruitSorting.pptx` | Output | Generated presentation |

---

### Task 1: Create script skeleton with helpers + Slide 1 (Title)

**Files:**
- Create: `slides/generate_final_pptx.py`

- [ ] **Step 1: Create `slides/` directory and script with helpers + Slide 1**
...
# ── Save placeholder (will be replaced as slides are added) ──
...
- [ ] **Step 2: Run the script to verify Slide 1 generates**

Run:
```bash
mkdir -p slides
C:/Python313/python.exe slides/generate_final_pptx.py
```

Expected: `Saved to: .../slides/FinalPresentation_FruitSorting.pptx` — open in PowerPoint to verify Slide 1 layout.

- [ ] **Step 3: Commit**

```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): scaffold final presentation script + slide 1 (title)"
```

---

### Task 2: Slide 2 — System Architecture

**Files:**
- Modify: `slides/generate_final_pptx.py` (append before the save block)

- [ ] **Step 1: Add Slide 2 code**

Insert before the `_out = ...` / `prs.save(...)` lines:
...
- [ ] **Step 2: Run script, verify Slides 1-2**
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 2 — system architecture pipeline"
```

---

### Task 3: Slide 3 — Calibration & Vision

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 3 code**

Insert before the save block:
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 3 — calibration & vision pipeline"
```

---

### Task 4: Slide 4 — Control & Trajectory

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 4 code**
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 4 — control, IK, trajectory & FSM"
```

---

### Task 5: Slide 5 — GUI & Voice Control

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 5 code**
...
add_paragraph(tf_voice,
              '4. "stop" always dispatched (no wake needed)', 10,
              color=ACCENT_GREEN)
add_paragraph(tf_voice, "", 4)
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 5 — GUI & voice control"
```

---

### Task 6: Slide 6 — Results & Validation

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 6 code**
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 6 — results & validation"
```

---

### Task 7: Slide 7 — Challenges & Feedback

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 7 code**
...
```bash
git add slides/generate_final_pptx.py
git commit -m "feat(slides): add slide 7 — challenges, solutions & feedback"
```

---

### Task 8: Slide 8 — Conclusion & Future Work + Final Verification

**Files:**
- Modify: `slides/generate_final_pptx.py`

- [ ] **Step 1: Add Slide 8 code**
...
- [ ] **Step 2: Run full script and verify all 8 slides**

```bash
C:/Python313/python.exe slides/generate_final_pptx.py
```

Expected: `Saved to: .../slides/FinalPresentation_FruitSorting.pptx` — open in PowerPoint and verify all 8 slides render correctly with dark navy theme, proper layout, auto-inserted images where available, and placeholder boxes elsewhere.

- [ ] **Step 3: Final commit**

```bash
git add slides/generate_final_pptx.py slides/FinalPresentation_FruitSorting.pptx
git commit -m "feat(slides): complete 8-slide final presentation generator"
```

---

Plan complete and saved to `docs/superpowers/plans/2026-04-29-final-presentation.md`. Two execution options:
